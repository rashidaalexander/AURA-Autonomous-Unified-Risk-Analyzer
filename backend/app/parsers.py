...
def _parse_pyproject(text: str) -> List[str]:
    # minimal/best-effort parse:
    # grabs dependency-like strings such as: fastapi>=0.100, "uvicorn[standard]==0.30.1"
    pkgs = set()
    for line in text.splitlines():
        s = line.strip()
...
def detect_and_extract_packages(filename: str, raw: bytes) -> Dict[str, Any]:
    ...
    if not pkgs:
        return {
            "ok": False,
            "error": f"No packages detected in {kind}.",
            "hint": "Make sure the file contains dependencies.",
        }
